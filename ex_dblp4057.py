...
if not os.path.exists(root_path+'pre_trained'):
    os.makedirs(root_path+'pre_trained')
checkpt_file_Q = root_path+'pre_trained/%s_allMP_multi_%s_'%(dataset, featype) +'/%d/'

embedding_Q =root_path+'jhy_final_embedding%d.npy'
# training params
K = 5
batch_size = 1
nb_epochs = 50 #zwt200
...
processed_data_path='processed_data/%s/DBLP4057_GAT_with_idx.mat' %dataset
...
def analysis_data_dblp(data):
    truelabels, truefeatures = data['label'], data[features_name].astype(float)

    N = truefeatures.shape[0]
    rownetworks = [data['net_APCPA'] - np.eye(N), data['net_APA'] - np.eye(N)]  # , data['PTP'] - np.eye(N)]
...
for k in range(1,K+1):
    os.makedirs(checkpt_file_Q % k)
    checkpt_file= checkpt_file_Q % k +'%d.ckpt' %k
    print('model: {}'.format(checkpt_file ))
    # use adj_list as fea_list, have a try~
    data = sio.loadmat(processed_data_path)
    if k>1:
        data[features_name]= np.load(embedding_Q % (k-1))
    print(data)
    adj_list, fea_list, y_train, y_val, y_test, train_mask, val_mask, test_mask = analysis_data_dblp(data)
    if featype == 'adj':
        fea_list = adj_list
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            while ts_step * batch_size < ts_size:
                ...
                np.save(embedding_Q % k,  data[features_name])
